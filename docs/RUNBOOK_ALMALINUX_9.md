...
Начальные production defaults polling оставить:
...
После первого запуска администраторы могут изменить normal-poll диапазон через кнопку `Интервал`; сохранённое значение в SQLite становится runtime source of truth. `SOMON_POLL_MIN/MAX` остаются начальными defaults для новой или legacy settings-записи.
...
6. следующий poll назначается случайно по сохранённому диапазону, по умолчанию через 10–30 минут.
