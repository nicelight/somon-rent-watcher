...
В `/filter` кнопка `Интервал` принимает диапазон минут (`10-30`) или одно фиксированное значение (`15`). `Сканировать сейчас` временно меняется на `Сканирую...` и ставит poll в очередь в том же процессе; при нулевой активной выдаче бот присылает итоговое сообщение. При выключенном мониторинге scan только обновляет baseline, а активный HTTP backoff не обходится.
