...
- настраиваемый через Telegram случайный polling, по умолчанию раз в 10–30 минут;
...
Локальный запуск на Kubuntu через один Docker Compose service описан в [local Docker runbook](docs/RUNBOOK_DOCKER_LOCAL.md). Контейнер не публикует ports и хранит SQLite в named volume.

## Telegram-фильтр

Настраивать сервис могут только Telegram-пользователи из `TELEGRAM_ADMIN_USER_IDS`. Команды работают в личном чате с ботом и в настроенной целевой группе; все администраторы изменяют один общий фильтр:
...
- кнопка «Интервал» — задать случайный диапазон в минутах (`10-30`) или фиксированный период (`15`);
- кнопка «Сканировать сейчас» — поставить немедленный poll в очередь без параллельного запуска и без обхода backoff; на время цикла она показывает «Сканирую...», а нулевой результат подтверждается отдельным сообщением.

В группе бот игнорирует обычных участников, администраторов из других групп и обычный текст вне ожидаемого ввода. Для ввода цены и минус-слов в группе отключите Group Privacy через `@BotFather`; доступ к управлению всё равно проверяется по ID.
